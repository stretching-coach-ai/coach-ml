...
class MetadataGenerator:
    ...
    def create_content_based_prompt(self, exercise: Dict[str, Any], muscle_name: str, muscle_info: Dict[str, Any]) -> str:
        """콘텐츠 기반 메타데이터 생성을 위한 프롬프트 생성
        ...
        # 프롬프트 생성
        prompt = f"""
        당신은 근골격계와 스트레칭 전문가입니다. 다음 학술 논문은 직접적인 스트레칭 프로토콜을 제공하지 않지만, 
        관련 근육과 상태에 대한 중요한 정보를 포함하고 있습니다. 이 정보를 바탕으로 적합한 스트레칭 방법을 생성해주세요.
        ...
        이 논문의 내용(근육 기능, 영향, 상태 등)을 기반으로, 다음 정보를 JSON 형식으로 제공해주세요:
        
        1. 논문_분석:
           - 주요_발견사항: 논문의 핵심 발견 사항
           - 관련_근육_기능: 논문에서 설명하는 근육의 기능
           - 관련_증상_패턴: 논문에서 언급된 증상/패턴
           - 잠재적_개입_지점: 스트레칭이 도움될 수 있는 부분
        
        2. 권장_스트레칭_프로토콜:
           - 권장_스트레칭_유형: 이 근육/상태에 적합한 스트레칭 유형
           - 스트레칭_목적: 이 스트레칭의 목적과 기대 효과
           - 시작_자세: 상세한 시작 자세
           - 동작_단계: 단계별 상세 지침 (최소 5단계)
           - 근거_설명: 이 스트레칭을 권장하는 근거 (논문 내용 기반)
